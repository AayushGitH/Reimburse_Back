...
public class EmployeeControllerTest {
  ...
  void get_assignUserManager() throws Exception {
  	...
  	assignEmployeeInDto.setEmployeeEmail(userEmail);
  	assignEmployeeInDto.setManagerEmail(managerEmail);
  	
  	// Act
  	when(userService.assignEmployee(userEmail, managerEmail)).thenReturn("Successfully assigned user with manager");
  	ResultActions response = this.mockMvc
  			.perform(post("/employee/assign")
  		    .content(this.objectMapper.writeValueAsString(assignEmployeeInDto))
  			.contentType(MediaType.APPLICATION_JSON));
  	response.andExpect(MockMvcResultMatchers.status().isOk());
  }
}
