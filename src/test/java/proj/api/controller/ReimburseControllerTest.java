...
//  	Reimburse reimburse = new Reimburse(101, "FOOD", 1200, new Date(), "URL", "INR", "Comment", Status.PENDING, null, null);
	...
	final ResultActions response = mockMvc
      .perform(multipart("/claim/create/test@nucleusteq.com").file(file)
      .param("claim", reimb));
  ...
  void testViewClaims() throws Exception {
  	...
  	ResultActions response = this.mockMvc
  			.perform(get("/claim/viewClaims")
  		    .param("email", "test@nucleusteq.com")
  ...
  void testGetAllClaimsByStatus() throws Exception {
	...
	Date date = new Date();
  	...
  	ResultActions response = this.mockMvc
  			.perform(get("/claim/getClaimsByStatus")
  ...
  void testGetAllClaimsForManagerStatus() throws Exception {
  	...
  	ResultActions response = this.mockMvc
  			.perform(get("/claim/getClaimsManagerStatus")
  			.param("managerId", "101")
  			.param("status", "Pending")
  			.contentType(MediaType.APPLICATION_JSON));
  	response.andExpect(MockMvcResultMatchers.status().isOk());
  ...
  void testGetSpecificClaims() throws Exception {
  	...
  	ResultActions response = this.mockMvc
  			.perform(get("/claim/viewUserClaims")
  			.param("email", "test@nucleusteq.com")
  			.param("status", "Accepted")
  			.contentType(MediaType.APPLICATION_JSON));
  	response.andExpect(MockMvcResultMatchers.status().isOk());
  ...
  void testGetImage() throws Exception {  	
  	// Act
  	ResultActions response = this.mockMvc
  			.perform(get("/claim/image/1697542957024_foodBillImage.jpg")
  			.contentType(MediaType.APPLICATION_JSON));
  	response.andExpect(MockMvcResultMatchers.status().isOk());
  }
}
